...
my_file = open('third-level-asssociations-negative.tsv','r')
...
for s in spice_mapped_list:
    if j == 0:
        my_spice_file = open('Spice_negative.txt', 'w')
        my_spice_file.write(str(s.id) + ' ' + s.name + '\n')
        j += 1
    else:
        my_spice_file = open('Spice_negative.txt', 'a')
        my_spice_file.write(str(s.id) + ' ' + s.name + '\n')
j = 0
for d in disease_mapped_list:
    if j == 0:
        my_spice_file = open('Disease_negative.txt', 'w')
        my_spice_file.write(str(d.id) + ' ' + d.name + '\n')
        j += 1
    else:
        my_spice_file = open('Disease_negative.txt', 'a')
        my_spice_file.write(str(d.id) + ' ' + d.name + '\n')
...
for e in edge:
    if j == 0:
        my_edge_file = open('Edge_negative.txt', 'w')
        my_edge_file.write(str(e[0]) + ' ' + str(e[1]) + '\n')
        j += 1
    else:
        my_edge_file = open('Edge_negative.txt', 'a')
        my_edge_file.write(str(e[0]) + ' ' + str(e[1]) + '\n')
