...
for s in spice_mapped_list:
    if j == 0:
        my_spice_file = open('Spice(463-610).txt', 'w')
        my_spice_file.write(str(s.id) + ' ' + s.name + '\n')
        j += 1
    else:
        my_spice_file = open('Spice(463-610).txt', 'a')
        my_spice_file.write(str(s.id) + ' ' + s.name + '\n')
j = 0
for d in disease_mapped_list:
    if j == 0:
        my_spice_file = open('Disease(1-462).txt', 'w')
        my_spice_file.write(str(d.id) + ' ' + d.name + '\n')
        j += 1
    else:
        my_spice_file = open('Disease(1-462).txt', 'a')
        my_spice_file.write(str(d.id) + ' ' + d.name + '\n')
...
for e in edge:
    if j == 0:
        my_edge_file = open('Edge(disease 1-642).txt', 'w')
        my_edge_file.write(str(e[0]) + ' ' + str(e[1]) + '\n')
        j += 1
    else:
        my_edge_file = open('Edge(disease 1-642).txt', 'a')
        my_edge_file.write(str(e[0]) + ' ' + str(e[1]) + '\n')
